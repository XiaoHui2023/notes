...
- **决议**：移除 **Excalidraw / diagram** 配图方案：删除 **`tools/excalidraw/`**、**`.cursor/skills/excalidraw-diagram/`**、小仓库 **`diagrams/`**、**`assets/`** 生成图及 **`library-showcase.md`**。
- **要求**：讲义改表/列表/目录树或外链图；**`notes-convention`**、**`project-design-notes`**、**`project-preload-skills`** 已去掉 Excalidraw 口径。

## 2026-05-24

- **决议**：**`tools/`** 放在 **git 仓库根**（`notes/tools/excalidraw/`），不再放在仓库外 `Project/tools/`。**（2026-05-25 已撤销：整目录删除。）**

- **决议**：**文档内容**在 **`src/`**（如 `src/ai/instruction/`）；git 仓库根不设 `index.md`。
- **要求**：小仓库内不建 `tools/`。

- **决议**：小仓库配图仅 **`assets/*.png`** + **`diagrams/*.excalidraw`**；**不用 Mermaid** 默认配图。**（2026-05-25 已撤销：不再维护 Excalidraw 源稿与导出 PNG。）**
